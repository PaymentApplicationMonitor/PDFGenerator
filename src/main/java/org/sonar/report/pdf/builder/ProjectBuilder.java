...
public class ProjectBuilder {
  ...
  private void initMostViolatedRules(final Project project) throws IOException,
    ...
    LOG.info("Accessing Sonar: getting most violated rules");
    ...
    for (int i = priorities.length - 1; i >= 0 && limit > 0; i--) {
      ...
      // query.setExcludeRuleCategories(true);
        LOG.info("key " + project.getKey());
        LOG.info("url path "+UrlPath.getViolationsLevelPath(priorities[i]));
      ...
      if (mostViolatedRulesByLevel != null) {
        int count = initMostViolatedRulesFromNode(project,
          ...
          LOG.info("\t " + count + " " + priorities[i] + " violations");
        limit = limit - count;
      } else {
          LOG.info("There is not result on select //resources/resource");
          LOG.info("There are no violations with level " + priorities[i]);
      }
    }
  }
